...
public final class SettingsMenu {
    ...
    public static final DynamicMenuBuilder<Match> desc = new DynamicMenuBuilder<>(I18n.get("menu_desc_title"), 6,
            (builder, player, match) -> {
                ...
                builder.setSlot(4, 2, PLAYER_HEAD, I18n.get("menu_desc_scanplayer"), scanLore(match))
                        .setLeftClickAction((p, view) -> {
                            if (match.isInProgress()) {
                ...
                if (match.isInProgress()) {
                    ...
                    boolean ready = isConfigured(data) && match.getMatchState() == MatchState.READY;
    ...
    private static final DynamicMenuBuilder<Void> settingsBuilder = new DynamicMenuBuilder<>(I18n.get("menu_title"), 6,
        ...
        builder.setSlot(4, 0, NETHER_STAR, "§bCubeBall §f管理中心",
    ...
    private static boolean isConfigured(MatchData data) {
        return data.ballSpawn != null && !data.blueTeamSpawns.isEmpty() && !data.redTeamSpawns.isEmpty()
                && data.hasBlueTeamGoalArea() && data.hasRedTeamGoalArea();
    }
    ...
    private static String startLore(MatchData data, Match match) {
        if (isConfigured(data) && match.getMatchState() == MatchState.READY) {
            return "§a场地与队伍已就绪\n§e左键 §7开始比赛";
        }
        return "§c尚未满足开赛条件\n§7需要完成全部场地配置\n§7并扫描出生点附近玩家";
    }
